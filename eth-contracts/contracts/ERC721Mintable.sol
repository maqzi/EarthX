...
contract Ownable {
    ...
    /**
        * @dev Emitted whenever ownership is transferred successfully from `oldOwner` to `newOwner`.
        */
...
contract ERC721 is Pausable, ERC165 {
    ...
    // TIP: remember the functions to use for Counters. you can refresh yourself with the link above
    function _mint(address to, uint256 tokenId) whenNotPaused internal {

        // TODO revert if given tokenId already exists or given address is invalid
        if(address(to) == address(0) || _exists(tokenId)){
            revert("Invalid address or token exists");
        }
...
contract EarthX is ERC721Metadata{
